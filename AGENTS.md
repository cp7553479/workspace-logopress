...
`接收/恢复 → 定义终态 → 规划并建立 DAG → 分步执行 → 自进化与反思 → Final Reply`

### 1. 接收或恢复

判断当前消息是新任务、澄清答复、任务补充、状态询问还是目标替代。恢复已有任务时，读取必要的进度记录和 artifact，复用已经验收的结果；新指令改变目标时，调整尚未执行的计划。
...
明确用户希望达成的目标、交付物、范围、约束、权限边界、验收标准、允许的副作用和退出条件。会实质影响目标、范围、成本、权限或风险的选择由用户确认；格式细节沿用已确认的用户偏好和 workspace 约定。

当用户要求持续追踪，或明确要求将某个目标建立为 goal 时，use `create_goal` tool to record the confirmed objective and acceptance criteria。

若当前存在 active goal，且目标实际达到验收标准时，use `update_goal` tool to mark it complete；仅在该工具定义的 blocked 条件满足时标记 blocked。

复杂、多步骤、跨轮或需要恢复的任务，按需把目标、关键决策和进展记录或迭代到 `memory/YYYY-MM-DD-<task name>.md`。

### 3. 规划并建立 DAG

对任务整体结构和依赖关系保持清晰判断，使复杂任务能够被合理拆解、持续跟踪并顺利推进；需要维护多步骤计划时可使用 `update_plan`。

多 subagent 长任务应具备跨轮恢复能力，必要的计划、进度和下一步可保存在 `.temp/<task-space>/process.json` 中。
...
把 subagent 视为另一位可对话、可独立执行的协作者；其能力以会话中实际可调用的工具、工作区、上下文和 skills 为准。可按当前会话支持的方式派生 subagent，并用 `sessions_send` 向既有 subagent 会话追加或调整指令。

### Subagent工作区

- 派生方式支持指定工作目录时，可按任务需要设置 `cwd`；Codex native runtime 可按原生规则读取该目录层级的 `AGENTS.md`。
- 任务需要使用角色目录中的专属 `AGENTS.md` 和 `skills/` 时，将该目录配置为对应 `agentId` 的 workspace，并通过该 `agentId` 派生；仅设置 `cwd` 用于指定工作目录。
- 临时或通用任务可使用当前工作区。
...
`AGENTS.md` 定义该角色的职责、workflow 和规则；`skills/` 保存该角色专用 skills。

### 派生subagent规范

- 派生 Task 建议按任务需要说明目标、输入与依赖、范围与读写边界、输出、证据、验收及失败策略，并只传递完成节点所需的最小上下文。
- subagent 可在实际能力范围内自主读写文件、识图、使用 skills、执行代码和调用工具；Task 可直接要求其读取指定文件或使用指定 skill，无需复述文件内容或 skill 工作流程。互联网搜索使用精简 brief；
...
sessions_spawn({
  runtime: "subagent",
  mode: "run",
  cwd: "<workspace_path>", // 可选
  taskName: "<name>",
  task: "<task description>"
})
```

`sessions_yield({ message: "<waiting reason or handoff>" })`：等待完成事件或交接。

`sessions_send({ sessionKey: "<subagent_session_key>", message: "<instruction>"})`：向既有会话追加指令，避免重复派生。
...
```text
wiki_status({})
wiki_search({query: "<entity-or-topic>", corpus: "wiki"})
wiki_get({lookup: "<page>"})
wiki_apply({op: "synthesis", title: "<title>", body: "<source-backed synthesis>", sourceIds: ["<source-id>"]})
wiki_lint({})
```
