...
- 仅 Main Session 可筛选候选商品、选定任务和对外发送。派发前必须固定并记录单一产品编号、`record_id`、当前远端记录版本及`参考图片`附件 token，然后为该产品建立唯一占用锁。
- handoff 必须明确写明唯一允许处理的 LP 编号和 `record_id`。subagent 不得自行筛选、替换或猜测商品，不得触碰任何其他记录；仅向 Main Session 回报文件路径、附件 token、验收证据和 blocker，不得对用户或外部会话发送交付内容。
- 同一产品任一时刻只允许一个执行流程。重试前必须确认旧流程已结束，并由 Main Session 完成验收/状态记录后释放该产品占用锁。
- 同时最多派发两个任务，且必须是两个不同、已分别锁定的产品。写入前后均须回读 Base；远端基线或参考 token 变化时停止写入并回报冲突。
- subagent 一旦产生合格图片即须立即向 Main Session 回报，不得等待整套完成。Main Session 必须先独立回读飞书多维表格，确认相应字段与附件 token 已确实上传且符合预期，随后立即向用户发送该图片；每次发送均记录成功或失败证据。
- Base 回读不一致，或图片上传/IM 发送因权限或其他原因失败时，Main Session 必须立即向用户明确报告，不得声称已交付；记录证据后可继续处理其余图片和任务。
- subagent 完成后，Main Session 必须立即独立回读验收、更新状态、释放占用锁，并在有空闲槽位时补派下一条；Main Session 必须等待正在处理的 subagent 返回，不能在派出后直接结束当前调度循环。
...
- subagent 正常完成属于当前 DAG 的完成事件，应优先由 Main Session 在同一 LOOP 内等待并立即处理，不属于“未来时点恢复”。不得因为已经配置 Heartbeat 就结束仍可继续推进的 LOOP。
