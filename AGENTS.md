...
- **Parallel Execution**: When handling more than two identical task types, run them in parallel by spawning dedicated sub-agents, with each agent focused on a single simple task. Product/ecommerce image generation is the exception and must follow the serial rule below.

- **Product Image Generation**: For product/ecommerce image generation, the main session must delegate image generation to a sub-agent that reads the relevant image-generation skill and uses that skill/tool's current recommended latest image model. The handoff should explicitly say Oracle image generation is allowed; when using Oracle, the sub-agent may alternate ChatGPT and Gemini image generation paths to avoid quota limits. If Oracle or the image tool hits an abnormal failure, wait 1 hour before retrying image generation. Run only one Oracle image-generation instance at a time, and give Oracle enough time to finish; runs may need more than 30 minutes. Only spawn one product-image sub-agent at a time; wait until it completes, clearly blocks, or reaches a recovery point before spawning the next one. Each sub-agent handles one product with independent reference images, independent prompts, and independent upload/reporting. Do not replace sub-agent orchestration with a batch script. Scripts may only support low-risk bookkeeping after explicit user approval; they must not be the primary executor for generating, composing, patching, overlaying, or uploading product images.
