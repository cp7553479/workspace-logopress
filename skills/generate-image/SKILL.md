...
- 必须传入 `streamTo: "parent"`；`parent` 是固定字符串值，不要替换成 channel、chat_id、sessionKey 或其他值。
...
```json
{
  ...
  "streamTo": "parent",
  "cwd": "<当前任务专用临时目录绝对路径>",
  "task": "You are in <当前任务专用临时目录绝对路径>. First read ./AGENTS.md if it exists, then inspect the reference images before generation. Use built-in image generation capability only. Do not use scripts, PIL, ImageMagick, SVG/canvas, local compositing, or programmatic editing to create the final image.\n\nReference image 1: <参考图1绝对路径>, this reference is <参考图1用途>.\nReference image 2: <参考图2绝对路径>, this reference is <参考图2用途>.\nOptional logo reference: <Logo文件绝对路径>, this is the required logo asset and must not be replaced by a generic logo.\n\nGenerate requirements: <英文生图提示词，必须写清楚图片类型、商品细节、1:1比例、禁止项、是否需要Logo/文案>.\n\nSave/copy the final generated image exactly to: <输出图片绝对路径>.",
