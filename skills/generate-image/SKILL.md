---
name: generate-image
description: 当用户想要生成图片或修改现有图片时使用。
---
...
- 生成成功后，使用 `message` 把图片文件直接发给用户，在`message`工具里不要提本地文件路径
...
- 必须设置：`runtime: "acp"`、`agentId: "codex"`、`mode: "session"`、`thread: true`、`streamTo: "parent"`。
