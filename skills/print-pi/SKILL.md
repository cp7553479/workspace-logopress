...
- **PI表:** `tbl78pgySVkHfcg3`（索引字段 `PI`，Text类型，值为PI编号如 `LP26041701`）
- **PI明细表:** `tbl4LsNziiCNGv5z`（主键 `商品编号`，Text类型；关联字段 `PI` 指向PI表）
...
feishu_bitable_list_records(app_token="KcXMbMUvAa7TNYsIqD7cwNi1nDf", table_id="tbl78pgySVkHfcg3")
...
```python
# 商品明细字段值结构:
# fields["商品明细"] = [{"record_ids": ["recxxx", ...], "table_id": "tbl4LsNziiCNGv5z"}, ...]
# 收集所有 record_id，逐条读取
feishu_bitable_get_record(
    app_token="KcXMbMUvAa7TNYsIqD7cwNi1nDf",
    table_id="tbl4LsNziiCNGv5z",
    record_id=<record_id>
)
```
...
- DuplexLink字段（商品明细）值结构为 `[{record_ids: [...], table_id: "tbl4LsNziiCNGv5z"}]`，需提取 record_id 逐条读取
