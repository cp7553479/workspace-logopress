...
3. 先处理参考图净化：如果后续需要作为输入的参考图含有 logo、sample logo、示例印刷、品牌字样、二维码、水印、厂家/供应商/平台标识、联系方式或网址，必须先用生图 skill 生成净化版参考图，只消除这些标记，其他产品与画面信息保持不变；净化版保存到 `references/cleaned/`，并从此替代原图参与后续白底图、首图和 ASI 套图生图。
...
8. 为每一张待生成图片单独编写英文 Prompt（可使用webSearch搜索类似产品的文案参考案例）。
...
10. 当前商品完成后，update_plan 中记录任务状态、已生成文件、Prompt 记录与跳过原因。
