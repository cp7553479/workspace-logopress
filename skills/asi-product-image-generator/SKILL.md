...
`商品线索表` 是以 `产品编号` 为记录的索引（如果有 `record_id` 也可以作为索引），获取 `参考图片`、`白底图` 字段的附件作为参考图并下载到本地（按产品分文件夹分类好，避免文件交叉）。每个商品必须先在 workspace 的 `temp/` 目录下创建专用临时目录，用于存放 `AGENTS.md`、参考图、白底图、生成图和上传记录；目录名必须包含产品编号，建议格式为 `temp/<产品编号>_asi_<record_id>/`。生成后的主图/套图需传入到 `商品主图` 附件字段里。
...
- 首图参考图选择硬规则：优先只传当前产品白底图 + 指定 `LOGO-BK.png` / `LOGO-WT.png` 文件。不要混入普通参考图、历史生成图或其他素材，避免模型被其它素材干扰导致 Logo 错误；除非用户明确要求或白底图无法确认产品结构。
...
必须将logo文件作为参考图而不仅仅是文字描述，必须在提示词里写清楚使用该文件作为logo图。

**派生 subagent 时必须逐字强调：首图必须把指定 `LOGO-BK.png` 或 `LOGO-WT.png` 文件作为生图参考图传入，不允许只在 Prompt 里文字描述 logo；如果生图调用没有传入 logo 文件，则该首图任务视为未执行，必须重做。**

**首图参考图组合规则：首图生图优先只传当前产品白底图 + 指定 logo 文件，不要混入普通参考图、场景图、SKU 图或历史生成图，避免模型被其它素材干扰导致 logo 错误。只有白底图无法确认产品结构时，才可额外传 1 张最干净的产品参考图，并必须在 Prompt 中声明该图只用于产品结构，不用于 logo。**
