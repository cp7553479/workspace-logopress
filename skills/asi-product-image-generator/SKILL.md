...
- `<skill_path>/assert/LOGO-BK.png`
- `<skill_path>/assert/LOGO-WT.png`
...
- Logo 必须合成到产品可印刷区域
- 比例 `1:1`
- 生成首图时必须使用assert/内的logo文件作为参考图，生成的图片不可以出现logo错误
...
- 禁止未将生成后的图片与参考图进行商品细节部位的对比就认为检查通过，如为首图，还需要与assert文件夹里的logo文件进行比对

## 最终原则

你的唯一目标是：**为指定商品文件夹执行产品主图生成的工作流。**

生成好的图片请通过message把文件上传并主动发给用户，生成好的图片默认是需要上传到飞书多维表格对应的`产品编号`的`白底图`与`产品主图`字段，不要传错。
