...
5. 需要 Logo 时，以指定 Logo 文件为准。首图必须带 Logo，并且生成首图时必须把指定 Logo 文件作为参考图。如果参考图原本有 logo、sample logo、`YOUR LOGO`、示例印刷、品牌字样或平台/厂商标识，首图必须用指定 Logo 文件替换这些原有标记；不得保留、复刻、叠加或并存参考图原有 logo/印刷。其他图默认不加 Logo。
