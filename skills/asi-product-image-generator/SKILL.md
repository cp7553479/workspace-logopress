...
生成图片时，使用合适的生图 skill 生成图片，传入参考图，编写生图指令。每次调用必须传入产品参考图与英文提示词。你必须合理选择参考图以及设计提示词，建议 `1-3` 张白底图或除了有产品以外比较干净的图，如果是生成多 SKU 图可以传入 9 张参考图。若确因生图 skill 的参考图数量限制需要整理参考素材，只允许把多张参考图整理成“参考用 contact sheet”并压缩到 3MB 以内；该 contact sheet 只能作为生图参考输入，绝不能作为最终图、半成品图、Logo 贴图底稿或程序合成结果。当产品是可以拆卸或者可折叠时，并且你希望生成的图片是展示拆卸后或折叠后的图，请你选择对应的参考图（例如可拆卸的参考图和可折叠的参考图），然后在提示词里面明确指出参考 xxx 图的产品拆卸/折叠方式，生成 xxx 图。总之你必须决定要生成什么样的图片，提示词怎么写，使用哪些参考图，提示词里如何强化怎么参考提供的参考图。如果遇到问题，立马停止，并告诉我问题在哪里，由我选择下一步方案。
...
源素材图是指飞书多维表格的 `商品表` 的 `参考图片`、`白底图` 字段的附件，需要下载到本地。`参考图片` 字段可用于分析产品真实结构、颜色、材质、SKU，并作为白底图、首图或 ASI 套图的参考输入。若将用于后续生图输入的参考图上含有 logo、sample logo、示例印刷、品牌字样、二维码、水印、厂家/供应商/平台标识、联系方式或网址，必须先用生图 skill 生成一张“净化版参考图”：只移除这些可见标记，产品本体、角度、颜色、材质、结构、比例、背景和光影尽量保持不变。净化版参考图保存到当前商品目录的 `references/cleaned/`，并上传回当前记录的 `参考图片` 字段，替换带标记原图；回读确认后，后续白底图、首图和 ASI 套图都使用净化版参考图，不再使用带标记原图。禁止用脚本擦除、局部覆盖、修补、贴图或程序合成来处理参考图或最终交付图片。需要 Logo 时，以指定 Logo 文件为准，并在 Prompt 中要求使用指定 Logo 文件，不得复刻参考图原有 logo/印刷。如果 `白底图` 没有文件，则还需阅读 `generate-white-background` skill，先基于合适的产品参考图为该商品生成白底图（如果有多个颜色，每个颜色都需要生成一张白底图）。用户可能会直接发送图片给你，向用户确认该图片对应的“产品编号”，同样可以视为产品参考图。
...
派发商品图 subagent 时，handoff 必须明确要求 subagent 优先使用 Oracle 生图，并在 Oracle ChatGPT 与 Oracle Gemini 之间交替尝试；每张图轻量记录实际生图路径、输出文件和必要失败证据。只有当 Oracle ChatGPT 和 Oracle Gemini 都失败、不可用、额度/登录/模型/页面/网络/参考图支持受阻，或生成结果被拒绝后，才切换到 Codex 生图；切换前必须记录具体失败证据。
...
- 必须明确要求优先使用 Oracle 生图，并在 Oracle ChatGPT 与 Oracle Gemini 之间交替尝试；只有两条 Oracle 路径都失败、不可用或结果被拒绝并记录证据后，才切换到 Codex 生图。每张图只需轻量记录实际生图路径、输出文件和必要失败证据。
...
3. 先处理参考图净化：如果后续需要作为输入的参考图含有 logo、sample logo、示例印刷、品牌字样、二维码、水印、厂家/供应商/平台标识、联系方式或网址，必须先用生图 skill 生成净化版参考图，只消除这些标记，其他产品与画面信息保持不变；净化版保存到 `references/cleaned/`，上传回当前记录的 `参考图片` 字段并替换带标记原图，回读确认后再继续后续生图。
4. 检查 `白底图` 字段有没有文件，白底图是重要的参考图，如果 `白底图` 字段没有文件，必须先阅读 `generate-white-background` skill，并先基于合适的产品参考图或净化版参考图为该商品生成白底图、上传回 `白底图` 字段，再使用白底图和参考图继续生成其他图片。如果有多个 SKU，每个 SKU 都需要生成一张白底图。
5. 分析可用素材的内容、角度、背景、尺寸信息、材质信息、SKU 信息，以及是否适合作为白底参考图或主图参考图；实际套图生图输入必须使用当前商品的产品参考图或净化版参考图、白底图、已生成图和指定 Logo 文件。
6. 确认当前商品的产品描述、素材清单、参考图映射、生成计划与跳过原因。
7. 按图片类型规则判断当前商品需要生成哪些图。
8. 为每一张待生成图片单独编写英文 Prompt；仅在产品类型不熟、卖点文案不足或需要优化视觉方向时，使用 webSearch 搜索类似产品的文案或图片案例。
9. 先生成场景图、卖点图、尺寸图、材质工艺图、SKU 图等；首图必须最后生成，并使用产品参考图或净化版参考图、白底图、已生成的场景/卖点/材质/功能图以及 Logo 文件作为参考。若商品存在多个可靠 SKU/颜色，首图底部必须展示全部可靠 SKU/颜色；SKU 图可作为判断首图底部 SKU 展示的辅助参考，但不得作为首图右侧小图；尺寸图只能作为独立图片，不作为首图构图元素。
10. 当前商品完成后，轻量记录任务状态、已生成文件、Prompt 记录与跳过原因。
...
仅在产品类型不熟、卖点文案不足或需要优化视觉方向时，联网搜索 Amazon / 北美电商图片案例，辅助优化生图提示词。例如：
...
- 仅在产品类型不熟、卖点文案不足或需要优化视觉方向时，使用 webSearch 搜索并优化 Prompt。
