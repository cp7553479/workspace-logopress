...
面向北美促销礼品经销商业务，为指定的商品生成电商套图，并把结果写回 `商品主图` 字段。若你得任务有多个产品需要生成图片，可以派生多个subagent分别阅读本skill，然后处理指定产品，派生subagent直到每个subagent都只要求处理一个产品。
...
5. 需要 Logo 时，以指定 Logo 文件为准。首图必须带 Logo，并且生成首图时必须把指定 Logo 文件作为参考图。如果参考图原本有 logo、sample logo、示例印刷、品牌字样或平台/厂商标识，首图必须用指定 Logo 文件替换这些原有标记；不得保留、复刻、叠加或并存参考图原有 logo/印刷。其他图默认不加 Logo。首图的Logo实际上是一个logo示意图，即logo是带有`YOUR LOGO`字样的一个logo，文字`YOUR LOGO`不能省略（需要写进生图prompt里），公司名是`LogoPress`
...
8. 先生成场景图、卖点图、尺寸图、材质工艺图、SKU 图等；首图必须最后生成，并使用白底图、已生成的场景/卖点/尺寸/材质/SKU图以及 Logo 文件作为参考。
...
Logo 规则：需要 Logo 时，以指定 Logo 文件为准；参考图 Logo 与指定文件不同时，以指定文件为准。首图要明确要求“replace any existing/sample logo or imprint in the reference image with the specified LogoPress logo file”。Prompt 只说明使用指定 Logo 文件，不描述 Logo 外观。
