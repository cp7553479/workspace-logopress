...
源素材图是指飞书多维表格的 `商品表` 的 `参考图片`、`白底图` 字段的附件，需要下载到本地。`参考图片` 字段可用于分析产品真实结构、颜色、材质、SKU，并作为白底图、首图或 ASI 套图的参考输入。若参考图上的商品已有 logo、sample logo、示例印刷、品牌字样或平台/厂商标识，最终生成图不得复刻这些原有标记；需要 Logo 时，以指定 Logo 文件为准，并在 Prompt 中要求替换原有 logo/印刷。禁止用脚本擦除、局部覆盖、修补、贴图或程序合成来处理最终交付图片。如果 `白底图` 没有文件，则还需阅读 `generate-white-background` skill，先基于合适的产品参考图为该商品生成白底图（如果有多个颜色，每个颜色都需要生成一张白底图）。用户可能会直接发送图片给你，向用户确认该图片对应的“产品编号”，同样可以视为产品参考图。
...
3. 检查`白底图` 字段有没有文件，白底图是重要的参考图，如果 `白底图` 字段没有文件，必须先阅读 `generate-white-background` skill，并先基于合适的产品参考图为该商品生成白底图、上传回 `白底图` 字段，再使用白底图和参考图继续生成其他图片。如果有多个 SKU，每个 SKU 都需要生成一张白底图。
4. 分析每张图片的内容、角度、背景、是否含尺寸信息、是否含材质信息、是否为 SKU 图、是否适合作为白底参考图、是否适合作为主图参考图；实际套图生图输入必须使用当前商品的产品参考图、白底图、已生成图和指定 Logo 文件。
5. 确认当前商品的产品描述、素材清单、参考图映射、生成计划与跳过原因。
6. 按图片类型规则判断当前商品需要生成哪些图。
7. 为每一张待生成图片单独编写英文 Prompt（可使用webSearch搜索类似产品的文案参考案例）。
8. 先生成场景图、卖点图、尺寸图、材质工艺图、SKU 图等；首图必须最后生成，并使用产品参考图、白底图、已生成的场景/卖点/材质/功能图以及 Logo 文件作为参考。若商品存在多个可靠 SKU/颜色，首图底部必须展示全部可靠 SKU/颜色；SKU 图可作为判断首图底部 SKU 展示的辅助参考，但不得作为首图右侧小图；尺寸图只能作为独立图片，不作为首图构图元素。
