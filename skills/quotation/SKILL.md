...
目标：根据产品编号和数量读取产品信息并生成报价表，产品信息获取到了之后及时通过`message`工具发送，报价表的xlsx文件通过message工具发送。
...
使用`message` tool，以markdown表格形式将获取到的record关键字段的信息（包括商品编号，商品名称，箱规，全部的阶梯数量，全部的阶梯价，1688链接，1688价格）的原值发给用户
