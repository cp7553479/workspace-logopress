...
## Main use case (browser, GPT‑5.5)

Default workflow here: `--engine browser` with GPT‑5.5 (Auto/Thinking) in ChatGPT. This is faster than Pro and is the everyday path; expect a stored session you can reattach to.

Recommended defaults:

- Engine: browser (`--engine browser`)
- Model: GPT‑5.5 (use `--model gpt-5.5`; this targets ChatGPT's Auto/Thinking tier). Use `--model gpt-5.5-pro` only when you specifically need the Pro tier (slower, ~10 min–1 hr).
...
3. Run in browser mode for the usual GPT‑5.5 ChatGPT workflow; use API only when you explicitly want it.
...
- Browser run (main path; long-running is normal):
  - `oracle --engine browser --model gpt-5.5 -p "<task>" --file "src/**"`
  - `oracle --profile default --model gpt-5.5 -p "<task>" --file "src/**"`
...
- Run with a specific signed-in profile:
  - `oracle --profile default --model gpt-5.5 -p "<task>"`
  - `oracle --profile work --model gpt-5.5 -p "<task>"`
...
- Runs may detach or take a long time (Pro browser/API runs especially). If the CLI times out: don’t re-run; reattach.
